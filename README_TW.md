...
這是 **API 層**的頻率限制，針對所有介面呼叫，跟生成模式無關。超出後請求會被限流。

另外還有**生成並行限制**——每個 thread 同一時間只能執行一個生成任務。如果該 thread 已有任務在跑，新請求會被拒絕，需等當前任務完成。不同 thread 之間可以並行。
